...
select
  ...
  mb.id,
  mb.bill_month,
  (date_trunc('month', mb.bill_month) + interval '1 month')::date,
  greatest(mb.total_amount - coalesce(mb.transfer_amount, 0), 0),
  0,
  'open',
  '部分收款順延'
from public.monthly_bills mb
where mb.payment_status::text = 'partial_paid'
  and greatest(mb.total_amount - coalesce(mb.transfer_amount, 0), 0) > 0
